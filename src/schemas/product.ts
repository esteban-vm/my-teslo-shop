...
export const ProductForm = z.object({
  ...
  price: z.coerce.number().min(1, 'El precio mínimo es de 1$'),
  stock: z.coerce.number().int('Ingresa una cantidad entera').min(1, 'La cantidad mínima es de 1 unidad'),
  ...
  uploads: z
    .custom<FileList>()
    .refine((files) => files.length === 2, 'Selecciona 2 imágenes')
    .refine(
